...
| `Deployment Manager` (revised) | How a robot is actually brought onto the network in practice (SSH and a hand-loaded certificate file, not the documented "Activate Config" UI screen); how map draft/published versioning and fleet assignment really work (one map, one version, per fleet — switching requires unassigning everything first); that one server commonly hosts several independent fleets, scoped by login; and the team's own unshipped ask to move versioning from the map to the fleet. | Ati support engineer, Deployment Manager/Fleet Manager walkthrough, transcript supplied in conversation, September 2026 |
| `Dispatch` (revised), `Map annotation` (revised) | The three fixed, per-station dispatch options (not required / optional with timeout / required) and that this is a map-design-time setting, not something a trip booking in Fleet Manager can override; how stations and routes are actually placed (customer walkthrough plus test-driving); that routes are one-way and don't imply their reverse. | Ati support engineer, Deployment Manager/Fleet Manager walkthrough, transcript supplied in conversation, September 2026 |
| `Map creation` (revised), `Obstacle avoidance` (revised) | Map generation runs on the robot itself, with an internal GPU tool as fallback for maps too large to process on board; failures only ever surface a generic script error, not the cause; remapping after a physical site change only touches the affected area. Obstacle avoidance is enabled per rectangular zone with size/deviation/static-vs-moving parameters — outside such a zone the robot stops rather than swerving. | Ati support engineer, Deployment Manager/Fleet Manager walkthrough, transcript supplied in conversation, September 2026 |
| `Waypoint version` (new), `SKU` (revised) | New term for the underlying map/routing format (V2 point-to-point vs. V5 dynamic-approach), and the robot-type compatibility it fixes (tugger is V2-only, pallet mover is V5-only, lifter and Flex Tug support both) — forcing incompatible robot types onto separate fleets over the same site. Flagged, not merged: an informal use of "SKU" to mean robot type, distinct from the material-SKU already documented; and "V5" waypoints against Deployment Manager's own version 5.4 as a naming clash to avoid, not a confirmed link. Also logged that the full list of map zone types is undocumented even inside Ati. | Ati support engineer, Deployment Manager/Fleet Manager walkthrough, transcript supplied in conversation, September 2026 |
